...
public class CitasResource {
    ...
    public Response addCita(
        ...
        if (answ) {
            //return Response.temporaryRedirect(URI.create("http://localhost/proyectoDB2-Hospital1/citas_h.jsp?in=1")).build();
            return Response.status(200).type(MediaType.APPLICATION_JSON).entity("{\"in\":1}").build();
        } else {
            //return Response.temporaryRedirect(URI.create("http://localhost/proyectoDB2-Hospital1/citas_h.jsp?in=0")).build();
    ...
    public Response updateCita(
        ...
        if (answ) {
            //return Response.temporaryRedirect(URI.create("http://localhost/proyectoDB2-Hospital1/citas_h.jsp?up=1")).build();
            return Response.status(200).type(MediaType.APPLICATION_JSON).entity("{\"up\":1}").build();
        } else {
            //return Response.temporaryRedirect(URI.create("http://localhost/proyectoDB2-Hospital1/citas_h.jsp?up=0")).build();
    ...
    public Response deleteCita(
        ...
        answ = delCita(citaId);
        if (answ) {
            //return Response.temporaryRedirect(URI.create("http://localhost/proyectoDB2-Hospital1/citas_h.jsp?del=1")).build();
            return Response.status(200).type(MediaType.APPLICATION_JSON).entity("{\"del\":1}").build();
        } else {
            //return Response.temporaryRedirect(URI.create("http://localhost/proyectoDB2-Hospital1/citas_h.jsp?del=0")).build();
    ...
    private String[] getPorcentaje(long DPI) {
        ...
        try {//a
            // Send data
            URL url = new URL("http://localhost/proyectoDB2-Hospital1/GetCliente?dpi=" + DPI);
    ...
    private int instertAuth(String dateCita, String servicio, long DPI, int monto, String porcentaje, int cId) {
        ...
        try {//b
            // Send data
            String rStmt = "http://localhost/proyectoDB2-seguro/restAuth/auth/addAuth?hospital=" + hospitalNum + "&fecha=" + dateCita + "&servicio=" + servicio + "&dpi=" + DPI + "&monto=" + monto + "&porcentaje=" + porcentaje + "&idCita=" + cId;
            //String rStmt="http://localhost/proyectoDB2-Hospital1/GetCliente?dpi=" + DPI;
    ...
    private void insertFactura(int res, int cId, int monto, double pct) {
        ...
        if (res == 1) {
            try {
                // Send data
                URL urlauth = new URL("http://localhost/proyectoDB2-seguro/restAuth/auth/getAuth?idCita=" + cId + "&hospNum=" + hospitalNum);
